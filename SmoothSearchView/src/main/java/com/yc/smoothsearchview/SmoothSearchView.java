...
public class SmoothSearchView extends LinearLayout {
    ...
    private ImageView mSearchIconView; // 搜索框提示图标
    ...
    public SmoothSearchView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SmoothSearchView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context context) {
        View rootView = View.inflate(context, R.layout.smooth_search_view, this);
        this.mHorizontalScrollView = rootView.findViewById(R.id.horizontal_sv);
        this.mIconContainerLayout = rootView.findViewById(R.id.icon_container_layout);
        this.mSearchIconView = rootView.findViewById(R.id.search_icon);
        this.mSearchEt = rootView.findViewById(R.id.search_et);
        this.initListener();
    }
    ...
    private void removeViewAction(String iconTag) {
        ...
        if (this.mViewMap.size() == 0) {
            this.mSearchIconView.setVisibility(View.VISIBLE); // 显示搜索框提示图标
        }

        if (mOnIconRemoveListener != null) {
            mOnIconRemoveListener.onIconRemoved(SmoothSearchView.this, iconTag);
        }
    }
    ...
    public void setSearchHintIcon(@DrawableRes int resId) {
        if (this.mSearchIconView != null) {
            this.mSearchIconView.setImageResource(resId);
        }
    }
    ...
    public void hideSearchHintIcon() {
        if (this.mSearchIconView != null) {
            this.mSearchIconView.setVisibility(GONE);
        }
    }
